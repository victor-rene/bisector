...
class StickMan(RelativeLayout):
  ...
  def __init__(self, **kw):
    ...
    Clock.schedule_interval(self.constraint, 0.02)
  ...
  def flex(self):
    ...
    self.curr_anim = 'flex'
    self.anchor = None
    anim = Animation(angle=90, d=self.anim_duration)
  ...
  def high_punch(self):
    ...
    self.anchor = 'r_leg'
    anim = Animation(angle=45, d=self.anim_duration)
    anim.start(self.l_arm)
    anim.bind(on_complete=self.inc_anims)
    anim = Animation(angle=-90, d=self.anim_duration)
    anim.start(self.r_arm)
    ...
    anim.start(self.l_leg)
    anim.bind(on_complete=self.inc_anims)
    anim = Animation(angle=60, d=self.anim_duration)
    anim.start(self.r_thigh)
    anim.bind(on_complete=self.inc_anims)
    anim = Animation(angle=-90, d=self.anim_duration)
    anim.start(self.l_thigh)
  ...
  def reset(self):
    ...
    anim = Animation(angle=0, d=self.anim_duration)
    anim.start(self.trunk)
    anim.start(self.r_arm)
    anim.start(self.r_forearm)
    anim.start(self.r_thigh)
